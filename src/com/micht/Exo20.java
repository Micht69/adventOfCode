...
//			String s = str.substring(1, str.length() - 1);
			...
			this.x = Integer.parseInt(tab[0].trim());
			this.y = Integer.parseInt(tab[1].trim());
			this.z = Integer.parseInt(tab[2].trim());
		}

		public void add(Coord o) {
			this.x += o.x;
			this.y += o.y;
			this.z += o.z;
		}
	...
	private class Particule {
		...
		public Particule(int pId, String str) {
			this.id = pId;

			Matcher m = pattern.matcher(str);
			if (m.find()) {
				this.pos = new Coord(m.group(1));
				this.vit = new Coord(m.group(2));
				this.acc = new Coord(m.group(3));
				dist();
			} else {
		...
		private void dist() {
			dist = Math.abs(this.pos.x) + Math.abs(this.pos.y) + Math.abs(this.pos.z);
		}
	...
	private int program1(String... input) {
		List<Particule> particules = new ArrayList<>(input.length);

		for (int i = 0; i < input.length; i++) {
			Particule p = new Particule(i, input[i]);
			particules.add(p);
		}
		
		final Comparator<Particule> comp = (p1, p2) -> Long.compare(p1.dist, p2.dist);

		Particule closest = particules.stream().min(comp).get();
		int closestCount = 0;
		while (true) {
			particules.forEach(p -> p.move());
			Particule newClosest = particules.stream().min(comp).get();

			if (closest.id != newClosest.id) {
				closest = newClosest;
				closestCount = 0;
			} else {
				closestCount++;
				if (closestCount > 5000) {
					break;
				}
			}
		}
	...
	private int program2(String... input) {
		...
		while (true) {
			...
			particules.forEach(p -> p.move());
			// Remove collisions
			Set<Coord> allItems = new HashSet<>();
			Set<Coord> collisions = new HashSet<>();
			for (Particule p : particules) {
				if (allItems.contains(p.pos)) {
					collisions.add(p.pos);
				}
				allItems.add(p.pos);
			}
			List<Particule> distinctParticules = new ArrayList<>();
			for (Particule p : particules) {
				if (!collisions.contains(p.pos)) {
					distinctParticules.add(p);
				}
			}
			particules = distinctParticules;

			// Get closest
			Particule newClosest = particules.stream().min(comp).get();

			if (closest.id != newClosest.id) {
//				System.out.println(">>>>>> " + newClosest.id);
				closest = newClosest;
				closestCount = 0;
			} else {
				closestCount++;
