...
public class Exo02 {
	...
	public void example1_1() {
		int ret = program1("5	1	9	5", "7	5	3", "2	4	6	8");
	...
	private int program2(String... input) {
		...
		for (String line : input) {
			...
			List<Integer> valuesInt = values.stream().map(Integer::parseInt).collect(Collectors.toList());
			Collections.sort(valuesInt);

			for (int i = 0; i < valuesInt.size() / 2; i++) {
				int c1 = valuesInt.get(i);

				boolean found = false;
				for (int j = i + 1; j < valuesInt.size(); j++) {
					int c2 = valuesInt.get(j);
