...
public class Exo07 {
	...
	private class Prog1 {
		...
		public String toString() {
			return "Prog1 [name=" + name + ", weight=" + weight + ", tops=" + tops + ", parent=" + parent + "]";
		}
	}

	private String program1(String... input) {
		Map<String, Prog1> programs = new HashMap<>(input.length);
		...
		for (String line : input) {
			...
			if (m.find()) {
				Prog1 p = new Prog1();
				p.name = m.group(1);
				p.weight = Integer.parseInt(m.group(2));

				String topsStr = m.group(3);
				if (topsStr != null) {
					topsStr = topsStr.substring(4); // Remove ' -> '
					String[] topsTab = topsStr.split(", ");
					p.tops.addAll(Arrays.asList(topsTab));
				}
		...
		for (Prog1 p : programs.values()) {
			if (!p.tops.isEmpty()) {
				for (String n : p.tops) {
					programs.get(n).parent = p;
				}
			}
		}
		...
		for (Prog1 p : programs.values()) {
			if (p.parent == null) {
				if (bottom == null) {
					...
					System.err.println("Found multiple root");
					System.exit(1);
				}
			}
		}
	...
	private int program2(String... input) {
		Map<String, Prog2> programs = new HashMap<>(input.length);
		...
		for (String line : input) {
			...
			if (m.find()) {
				...
				if (topsStr != null) {
					topsStr = topsStr.substring(4); // Remove ' -> '
					String[] topsTab = topsStr.split(", ");
					p.topNames.addAll(Arrays.asList(topsTab));
				}
		...
		for (Prog2 p : programs.values()) {
			if (!p.topNames.isEmpty()) {
				for (String n : p.topNames) {
					Prog2 child = programs.get(n);
		...
		return checkWeight(bottom);
	}

	private int checkWeight(Prog2 parent) {
		Map<Integer, List<Prog2>> childsPerWeight = new HashMap<>();
		for (Prog2 child : parent.tops) {
			int wc = child.getWeight();

			List<Prog2> lc = childsPerWeight.get(wc);
			if (lc == null) {
				lc = new ArrayList<>();
				childsPerWeight.put(wc, lc);
			}
			lc.add(child);
		}

		int diff = 0;
		if (childsPerWeight.size() == 1) {
			// No différence
			return 0;
		} else {
			Iterator<Entry<Integer, List<Prog2>>> iterator = childsPerWeight.entrySet().iterator();
			Entry<Integer, List<Prog2>> e1 = iterator.next();
			Entry<Integer, List<Prog2>> e2 = iterator.next();
			...
			diff = checkWeight(p);
			if (diff == 0) {
				return p.weight - Math.abs(e1.getKey().intValue() - e2.getKey().intValue());
			}
		}
