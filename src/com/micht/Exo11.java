...
public class Exo11 {
	...
	private int program1(String input) {
		...
		return Math.max(Math.max(Math.abs(start.x), Math.abs(start.y)), Math.abs(start.z));
	}

	@Test
	public void example1_1() {
		int ret = program1("ne,ne,ne");
		Assert.assertEquals(3, ret);
	}

	@Test
	public void example1_2() {
		int ret = program1("ne,ne,sw,sw");
		Assert.assertEquals(0, ret);
	}

	@Test
	public void example1_3() {
		int ret = program1("ne,ne,s,s");
		Assert.assertEquals(2, ret);
	}

	@Test
	public void example1_4() {
		int ret = program1("se,sw,se,sw,sw");
		Assert.assertEquals(3, ret);
	}
	...
	private int program2(String input) {
		...
		for (String s : path) {
			...
			int d = Math.max(Math.max(Math.abs(start.x), Math.abs(start.y)), Math.abs(start.z));
			if (d > maxDist) {
				maxDist = d;
			}
		}
...
////		16f050a537234c7f348c21542f2acb74
//	}
//
//	@Test
