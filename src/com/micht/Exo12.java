...
public class Exo12 {
	...
	private class Prog {
		...
		public void add(int id2) {
			if (!connected.contains(id2)) {
				connected.add(id2);
			}
		}
	}

	private Prog getProg(Map<Integer, Prog> programs, int id) {
	...
	private int program1(String... input) {
		Map<Integer, Prog> programs = new HashMap<>();
		...
		for (String line : input) {
			...
			if (m.find()) {
				...
				Prog p = getProg(programs, id);

				String connected = m.group(2);
				if (connected != null) {
					String[] idsStr = connected.split(",");
					for (String idStr : idsStr) {
						int id2 = Integer.parseInt(idStr.trim());
						Prog p2 = getProg(programs, id2);
						p2.add(id);
		...
		Prog root = programs.get(0);
		founds.add(root.id);
		count(root, programs, founds);

		return founds.size();
	}

	private void count(Prog root, Map<Integer, Prog> programs, List<Integer> founds) {
		for (Integer pId : root.connected) {
			if (!founds.contains(pId)) {
				founds.add(pId);
				count(programs.get(pId), programs, founds);
			}
		}
	}
	...
	private int program2(String... input) {
		...
		int count = 0;
		List<Integer> founds = new ArrayList<>();
		for (Entry<Integer, Prog> entry : programs.entrySet()) {
			if (!founds.contains(entry.getKey())) {
				Prog root = entry.getValue();
				founds.add(root.id);
				count(root, programs, founds);
				count++;
			}
